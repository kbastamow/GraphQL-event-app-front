  ...
  return (
    <>
      <div className="d-flex justify-content-end">
        <button type="button" className="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#addEventModal">
          <div className="d-flex align-items-center">Edit Event</div>
        </button>
      </div>
      {loading || error ? (
        <></>
      ) : (
        <div className="modal fade" id="addEventModal" aria-labelledby="addEventModalLabel" aria-hidden="true">
                  ...
                  <form onSubmit={onSubmit}>
                    ...
                    <div className="d-flex flex-wrap">
                      {artists &&
                        artists.map((artist) => (
                          <span
                            className="badge bg-primary text-danger rounded-pill px-3 py-1 m-1 small"
                            key={artist.id}
                          >
                            {" "}
                            {artist.name}{" "}
                            <span onClick={() => setArtists(artists.filter((item) => item.id !== artist.id))}>
                              <BsFillTrash3Fill />
                            </span>
                          </span>
                        ))}
                    ...
                    <div className="d-flex justify-content-center">
                      <button type="submit" data-bs-dismiss="modal" className="btn btn-danger mt-2">
                        Save
                      </button>
                    </div>
                  </form>
                  ...
                  <div className="text-start small px-2 h-50 overflow-auto">
                    ...
                    <ul className="list-unstyled ps-3">
                      {data?.artists?.map((artist) => (
                        <li
                          key={artist.id}
                          className="artistList text-danger link"
                          onClick={() => setArtists([...artists, { id: artist.id, name: artist.name }])}
                        >
                          {artist.name}
                        </li>
                      ))}
