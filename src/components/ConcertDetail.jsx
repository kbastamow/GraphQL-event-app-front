  ...
  return (
    <>
          ...
          <div className="card-body">
            ...
            {!artists ? <></> : (
              artists.length === 1 ? (
                <p>Artist: <div onClick={() => showArtist(artists[0].id)} className="link ps-3"> <BsMusicNote className="pe-1 text-danger" /><span className="text-primary link">{artists[0].name}</span></div></p>
              ) : (
                <p>
                  Artists:
                  {artists.map((artist, index) => (
                    <div key={index} onClick={() => showArtist(artist.id)} className="link ps-3"> <BsMusicNote className="pe-1 text-danger" /><span className="text-primary link">{artist.name}</span></div>
                  ))}
                  <hr className="hr hr-blurry text-white"></hr>
                </p>
              )
